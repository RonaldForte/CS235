...
struct Quest{
    ...
    Quest():title_{""}, description_{""}, completed_{false}, experience_points_{0}
    {
        dependencies_ = {}; //previously had.clear() but just set it to an empty vect
        subquests_ = {};
    };

    //param construct
    Quest(const std::string& title, const std::string& description, bool completed, int experience_points, 
    const std::vector<Quest*>& dependencies, const std::vector<Quest*>& subquest)
    : title_{title}, description_{description}, completed_{completed}, experience_points_{experience_points},
    dependencies_{dependencies}, subquests_{subquest} {}
};
...
class QuestList : public DoublyLinkedList<Quest*>{
    ...
    */  
        bool addQuest(const std::string& title, const std::string& description, const bool& completed, const int& exp, const std::vector<Quest*>& dependencies, const std::vector<Quest*>& subquest);
        
    /**
    @param:  A Quest pointer
    @return: A boolean indicating if all the given quest's dependencies are completed
    */
        bool dependenciesComplete(Quest* depcomp);
    ...
    */ 
        bool questAvailable(Quest* avail);
    ...
    */
        int calculateProjectedExperience(Quest* domquest);
    ...
    */ 
        int calculatePathwayExperience(Quest* domquest);
    ...
    */
        void printQuestDetails(Quest* domquest);


    //My Helper functions for recursion hell :))
        void QueryHelp(Quest* queery);
        void QuestDetailHelp(Quest* quest, int indent);
};
