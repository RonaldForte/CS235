...
*/
void Mage::display()const{ //work on after TA checks barbarian
    ...
    std::cout << "They " << (can_summon_incarnate_ ? "can" : "cannot")
    << " summon an Incarnate." << std::endl << std::endl;
}
...
void Mage::eatTaintedStew(){
    if(getRace() == "UNDEAD"){ //pretty much the exact same as barbarian
        ...
        setVitality(1);
        if(getCastingWeapon() == "WAND"){ //adds vitality points if specific flags are true
