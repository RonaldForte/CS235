...
void Scoundrel::display()const{
    std::string scoundrel_enemy = "";
    std::string scoundrel_disguise = "";

    if (isEnemy() == true) {
        scoundrel_enemy = "an enemy.";
    } else {
        scoundrel_enemy = "not an enemy.";
    }

    if (hasDisguise() == 1) {
        scoundrel_disguise = "TRUE";
    } else {  // if it's not true
        scoundrel_disguise = "FALSE";
    }


    std::cout << getName() << " is a Level " << getLevel() << " " << getRace() << " " << "SCOUNDREL."  // printing
    << "\nVitality: " << getVitality()
    << "\nArmor: " << getArmor()
    << "\nThey are " << scoundrel_enemy
    << "\nDagger: " << getDagger()
    << "\nFaction: " << getFaction()
    << "\nDisguise: " << scoundrel_disguise << "\n\n"; 
}

void Scoundrel::eatTaintedStew(){
    if(getRace() == "UNDEAD"){ //pretty much the exact same as barbarian
