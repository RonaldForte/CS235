...
bool Character::isEnemy() const
{
    ...
    - The same race
    - The same level
    - They must either be both an enemy or not (same bool for enemy)
    */
bool Character::operator==(const Character& Ronald) const{
    if(name_ == Ronald.name_ && race_ == Ronald.race_ && level_ == Ronald.level_ && enemy_ == Ronald.isEnemy()){
        return true;
    }
    return false;
}
...
void Character::display() const{
    std::string temp = "";
    if(enemy_ == true){
        temp = "They are an enemy";
    }
    else{
        temp = "They are not an enemy";
    }
