...
*/
void Character::strike(Character* charac){
    if(charac -> getArmor() == 1){
        charac -> setArmor(0);
        charac -> setVitality(charac -> getVitality() - 1);
    }
    else if(charac -> getArmor() == 0 && charac -> getVitality() == 1){
        charac -> setVitality(0);
    }
    else if(charac -> getArmor() >= 2){
        charac->setArmor(charac -> getArmor() - 2);
    }
    else if(charac -> getArmor() == 0){
        charac -> setVitality(charac -> getVitality() - 2);
    }
}
...
*/
void Character::throwTomato(Character* charac2) {
    if (charac2 -> getArmor() >= 0) {
        charac2 -> setArmor(charac2 -> getArmor()-1);
    } 
    else{
        charac2 -> setVitality(charac2 -> getVitality() -1);
    }
    setVitality(getVitality() + 1);
}

bool Character::isBuffEmpty() const {
    return buff_.empty();
}
...
void Character::actionSelectionHelp2(int action){
  action_.push(action);
}

void Character::actionSelectionClear(){
  while(!action_.empty()){
    action_.pop();
  }
}